...
import { AdvertisementModel, userModel } from "../../src/models/exportModels";
import {
  AdvertisementRepository,
  RentRepository,
  UserRepository,
} from "../../src/repositories/exportRepositories";
...
const accManager = new userModel(userRepo);
...
describe("Database integration tests", () => {
  it("test #1", async () => {
    ...
    const ad1 = await adManager.addAdvertisement({
      description: "d1",
      cost: 111,
      address: "a1",
      ownerId: u1,
    ...
    const r1 = await adManager.newRent(
      ad1,
      u2,
      new Date(2023, 6),
      new Date(2023, 7)
  ...
  it("test #3", async () => {
    await expect(
      adManager.addAdvertisement({
        description: "d1",
        cost: 111,
        address: "a1",
        ownerId: "1",
      })
    ).rejects.toMatch(/doesn't exist/);
  });
});
